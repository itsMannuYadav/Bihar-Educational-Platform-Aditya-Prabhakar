...
async def generate_lesson_plan_node(state: TeachingKitState, config: RunnableConfig) -> dict:
    ...
    # Each Send-dispatched branch runs concurrently (docs/01-architecture.md
    # §3), so this node owns its own session rather than sharing one across
    # branches — see orchestrator.py's docstring for why that matters.
