...
_SAVED_COLS = select(
    ...
    TeachingKitRequest, SavedLesson.request_id == TeachingKitRequest.id
).join(
    Chapter, TeachingKitRequest.chapter_id == Chapter.id
).join(
    Subject, TeachingKitRequest.subject_id == Subject.id
).join(
    SchoolClass, TeachingKitRequest.class_id == SchoolClass.id
)
